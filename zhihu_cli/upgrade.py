...
def detect_upgrade_command() -> list[str] | None:
    ...
    if uv is not None and uv.returncode == 0 and DIST_NAME in (uv.stdout or ""):
        return ["uv", "tool", "upgrade", DIST_NAME]
    pipx = _run(["pipx", "list"])
    if pipx is not None and pipx.returncode == 0 and DIST_NAME in (pipx.stdout or ""):
...
def upgrade(*, assume_yes: bool = False) -> int:
    ...
    if cmd is None:
        raise ZhihuError(
            ...
            f"请手动运行 pip install --upgrade {DIST_NAME}"
        )
    if not assume_yes:
