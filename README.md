...
知乎命令行客户端。**所有命令都要求登录**，请求默认限速 1–3 秒，适合个人低频使用。
...
- 登录凭证以 JSON 明文保存在 `~/.config/zhihu-cli/credentials.json`，权限 `600`；
  保存与每次读取时都会自动收紧权限（防中途被改宽松）。
- 关于"明文"的风险水位：浏览器保存的知乎 Cookie 同样落在本机磁盘上（加密密钥也在本机），
  本工具的 `600` 明文文件与之处于同一水位——能读你 home 目录的攻击者两者都防不住。
  因此请勿在不可信或多用户的机器上使用；担心落盘可改用 `zhihu login` 后
  `logout` 及时清除，或直接不保存。
