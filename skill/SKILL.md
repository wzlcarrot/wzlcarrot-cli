...
- **配置目录**：`~/.config/zhihu-cli/`（可用环境变量 `ZHIHU_CLI_HOME` 覆盖）
  ...
  - 扫码登录二维码：`qrcode.png`
  - 会话：`sessions/`
- **登录方式**：`zhihu login`（默认弹出二维码，含有效期、过期自动刷新）；其它：`--link`、`--browser`、`--edge`、`--cookie "d_c0=...; z_c0=...; _xsrf=..."`。
...
3. **需登录**：先 `zhihu status`；未登录则 `zhihu login`（默认二维码）或 `--cookie`。
...
| 诉求 | 命令 |
|------|------|
| 登录（二维码/链接/浏览器/Edge/Cookie） | `zhihu login` / `--link` / `--browser` / `--edge` / `--cookie` |
